...
def annotate(
    items, sources, *, classes, history_length=5, display_value=None, cls=None, kwargs=None
):
    ...
    annotator = SourceAnnotator(classes, **kwargs)
...
class SourceAnnotator(Annotator):
    """IPython widget to annotate Sources.
    The widget expects a list of filenames.
    """
    def __init__(self, sources, classes, folder=None, **kwargs):
        if folder is None:
            self.folder = '.'
        else:
            self.folder = folder
        ...
        super(SourceAnnotator, self).__init__(classes, **kwargs)
        
    def build_display_value(self, item):
        source_info = self.sources.loc[item]
        source_name = get_source_name(source_info).replace(' ','_')
        lc_file = os.path.join(self.folder,'{}_lc.png'.format(source_name))
        cutout_file = os.path.join(self.folder,'{}_cutouts.png'.format(source_name))
        zoomed_cutout_file = os.path.join(self.folder,'{}_cutouts_zoom.png'.format(source_name))
        
        if os.path.isfile(lc_file):
            lc_html_str = '<img src="{}"</img>'.format(build_data_url(lc_file))
        else:
            lc_html_str = ''
        
        if os.path.isfile(cutout_file):
            cutout_html_str = '<img src="{}"</img>'.format(build_data_url(cutout_file))
        else:
            cutout_html_str = ''
        
        if os.path.isfile(zoomed_cutout_file):
            zoomed_cutout_html_str = '<img src="{}"</img>'.format(build_data_url(zoomed_cutout_file))
        else:
            zoomed_cutout_html_str = ''
        
        url_str = '<a href="https://dev.pipeline.vast-survey.org/sources/{}/" target="_blank" rel="noopener noreferrer">V_peak={:.3f}, Eta_peak={:.3f}, SNR: {:.1f}-{:.1f}</a></br>'.format(item,source_info.v_peak, source_info.eta_peak, source_info.min_snr, source_info.max_snr)
        return lc_html_str+url_str+cutout_html_str+zoomed_cutout_html_str
    
    def _get_stats(source_info):
        return

def annotate_sources(
    sources, classes, folder=None, history_length=5, display_value=None, cls=None, kwargs=None, annotations=None
):
    from IPython.core.display import display
    ...
    if annotations is not None:
        last_index = annotations[-1]['index']
        sources = sources[last_index+1:]
        
    annotator = SourceAnnotator(sources, classes, folder=folder, **kwargs)
    
    source_ids = list(sources.index)
    annotator.annotate(source_ids)
    display(annotator)
    return annotator.annotations, annotator
    
    #if annotations is None:
    #    return annotator.annotations
    #else:
    #    annotations.extend(annotator.annotations)
    #    return annotations